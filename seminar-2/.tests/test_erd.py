...
def test_entity_count(puml_content):
    ...
    assert len(entities) == 6, f"Ожидается 6 сущностей, найдено {len(entities)}"
...
def test_relationship_types(puml_content):
    ...
    assert one_to_many == 5, f"Ожидается 5 связей один-ко-многим, найдено {one_to_many}"

    many_to_many = len(re.findall(r'\}o--o\{', puml_content))
    assert many_to_many == 1, f"Ожидается 1 связь многие-ко-многим, найдено {many_to_many}"
